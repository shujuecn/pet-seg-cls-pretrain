...
class SegTrainer(BaseTrainer):
    ...
    def test_and_save(
        ...
        for batch in tqdm(loader, desc="[seg] test", leave=False):
            ...
            for i in range(pred.shape[0]):
                ...
                if save_original_size:
                    ...
                    if gt_mask.shape[::-1] != target_size:
                        gt_mask = np.array(
                            Image.fromarray(gt_mask).resize(target_size, resample=Image.NEAREST)
                        )
        ...
        dice_scores = dice_from_confusion(conf).cpu().numpy()
        iou_scores = iou_from_confusion(conf).cpu().numpy()
        metrics = {
            "loss": float(np.mean(losses)) if losses else 0.0,
        ...
        for idx in range(self.num_classes):
            metrics[f"dice_c{idx}"] = float(dice_scores[idx])
            metrics[f"iou_c{idx}"] = float(iou_scores[idx])
